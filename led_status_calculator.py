...
class LEDStatusCalculator:
    ...
    @staticmethod
    def get_status_for_airport(icao, airport_data, display_mode, current_forecast_hour, visited_airports):
        """
        ...
        Args:
            ...
            visited_airports (list): List of visited airport codes
        ...
        if airport_data:
            return LEDStatusCalculator._get_status_with_data(
                icao, airport_data, display_mode, current_forecast_hour, visited_airports
            )
        else:
            return LEDStatusCalculator._get_status_without_data(
                icao, display_mode, visited_airports
            )
    
    @staticmethod
    def _get_status_with_data(icao, airport_data, display_mode, current_forecast_hour, visited_airports):
        ...
        if display_mode == DisplayMode.METAR:
            ...
            return LEDStatusCalculator._get_visited_status(icao, visited_airports)
    ...
    @staticmethod
    def _get_status_without_data(icao, display_mode, visited_airports):
    ...
    @staticmethod
    def _get_visited_status(icao, visited_airports):
        """Get visited airport status"""
        is_visited = icao in visited_airports
